...
export default function PatientReviews() {
  ...
  useEffect(() => {
    axios.get('/api/review/get')
      .then(res => {
        setVideoReviews(res.data.filter(r => r.reviewType === 'video'));
        setTextReviews(res.data.filter(r => r.reviewType === 'text'));
      })
      .finally(() => setLoading(false));
  }, []);

  const startMarquee = (controls, items) => {
    const width = items.length * (CARD_WIDTH + GAP);
    controls.start({
      x: [0, -width],
  ...
  useEffect(() => {
    if (videoReviews.length > visibleCards)
      startMarquee(videoControls, videoReviews);
  }, [videoReviews, visibleCards]);

  useEffect(() => {
    if (textReviews.length > visibleCards)
      startMarquee(textControls, textReviews);
  }, [textReviews, visibleCards]);

  const containerWidth = visibleCards * (CARD_WIDTH + GAP) - GAP;
  ...
  return (
    <section className="max-w-7xl mx-auto px-4 py-16">
      ...
      {/* VIDEO REVIEWS */}
      <div className="mx-auto overflow-hidden mb-20" style={{ maxWidth: containerWidth }}>
        <motion.div className="flex gap-5" animate={videoControls}>
          {[...videoReviews, ...videoReviews].map((r, i) => (
            <div
              key={i}
            ...
            >
              <iframe
                ...
                src={`https://www.youtube.com/embed/${new URL(r.videoUrl).searchParams.get('v')}`}
                allowFullScreen
              />
              <div className="p-4 font-semibold">{r.reviewerName}</div>
            </div>
          ))}
        </motion.div>
      </div>
      ...
      {/* TEXT REVIEWS */}
      <div className="mx-auto overflow-hidden" style={{ maxWidth: containerWidth }}>
        <motion.div className="flex gap-5" animate={textControls}>
          {[...textReviews, ...textReviews].map((r, i) => (
            <div
              key={i}
              className="bg-gray-50 rounded-xl shadow-md p-6 w-[300px] flex-shrink-0 border border-gray-200"
            >
              ...
              <div className="flex items-center text-yellow-500 mb-2">
                ...
                {/* <span className="text-gray-500 ml-2">({r.rating})</span> */}
              </div>
