...
export default function AddBlog() {
    ...
    if (id) {
      axios
        ...
        .then((res) => {
          ...
          setExistingImages(blog.imagesArray || []);
          setSelectedCategory(`${blog.blogCategory}|${blog.blogCatID}`);

          fetchSubcategories(blog.blogCatID).then(() => {
            setSelectedSubcategory(
              `${blog.blogSubcategory}|${blog.blogSubCatID}`
            );
          });
        })
        .catch(() => Swal.fire("Error", "Failed to load blog", "error"))
        .finally(() => setLoading(false));
    } else {
      setLoading(false);
    }
  ...
  const fetchSubcategories = async (categoryId) => {
    try {
      const { data } = await axios.get(
        `/api/subcategory/get/categoryId/${categoryId}`
      );
      setSubcategories(data);
      return data;
    } catch {
      ...
      Swal.fire(
        "No Subcategories",
        "Please add subcategories first",
        "warning"
      );
      return [];
    }
  ...
  const handleCategoryChange = (e) => {
    ...
    setSelectedCategory(value);
    setSelectedSubcategory("");
    fetchSubcategories(value.split("|")[1]);
  };
  ...
  const handleSubmit = async (e) => {
    ...
    if (!blogTitle) validationErrors.blogTitle = "Title required";
    if (!blogContent) validationErrors.blogContent = "Content required";
    if (!selectedCategory) validationErrors.category = "Category required";
    if (!selectedSubcategory)
      validationErrors.subcategory = "Subcategory required";
    ...
    try {
      const res = id
        ? await axios.put(`/api/blogs/put/${id}`, payload)
        : await axios.post("/api/blogs/post", payload);

      Swal.fire("Success", "Blog saved successfully", "success");
      router.push(`/admin/blog-management/view-blog?id=${res.data._id}`);
    } catch {
  ...
  return (
    <div className="flex justify-center items-center min-h-screen bg-gray-100">
      <div className="w-full max-w-5xl bg-white p-7 rounded-lg shadow-lg">
        ...
        <form onSubmit={handleSubmit}>
            ...
            <div className="w-full">
              ...
              >
                ...
                {subcategories.map((sub) => (
                  <option
                    key={sub._id}
                    value={`${sub.subCategoryName}|${sub._id}`}
                  >
                    {sub.subCategoryName}
                  </option>
                ))}
          ...
          {/* CONTENT (FIXED LIST STYLES) */}
          <div className="mt-3">
            <label className="font-medium">Blog Content</label>

            <div className="blog-content border rounded-lg mt-1">
              <JoditEditor
                ref={editor}
                value={blogContent}
                onBlur={(newContent) => setBlogContent(newContent)}
            ...
            >
              {({ open }) => (
                <button
                  type="button"
                  onClick={() => open()}
                  className="bg-gray-600 text-white px-4 py-2 rounded-lg mt-2"
                >
                  Upload
                </button>
              )}
